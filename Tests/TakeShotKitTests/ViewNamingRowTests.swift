...
struct ViewNamingRowTests {
    ...
    @Test func theSlateRowMatchesTheFileRowsShape() async throws {
        try await ViewProbe.run { probe in
            ...
            // …and the block is one row plus the name preview under it —
            // because only one row shows, and the preview is one 9pt line
            // whatever the row is (owner: "чтоб под боксами было подписано оно
            // аккуратно"). Measured: 38pt of row, 52 of block, and the footer
            // 68 to 72.
            let block = probe.fittingSizes { NamingFieldsView() }
            let preview = NamingFieldsView.previewHeight
                + NamingFieldsView.previewGap
            #expect(abs(block.en.height - row.en.height - preview) <= 2, """
                the block is \(block.en.height)pt around a \(row.en.height)pt \
                row and a \(preview)pt preview
                """)
        }
    }
    ...
    @Test func theSlateRowSitsUnderTheNamingRowAndFitsTheFooterHalf() async throws {
        try await ViewProbe.run { probe in
            ...
            // the block really is the row plus the preview line under it
            let preview = NamingFieldsView.previewHeight
                + NamingFieldsView.previewGap
            #expect(abs(slate.en.height - slateRow.en.height - preview) <= 2,
