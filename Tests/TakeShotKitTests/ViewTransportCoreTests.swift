...
/// and they drifted: the glyph metrics of the play button still differ because
/// that difference is on screen. Everything else is now one implementation over
/// either engine, and these are the assertions that keep it that way — a piece
/// that measures differently depending on which engine drives it has grown an
/// engine-specific branch it should not have.
@MainActor
