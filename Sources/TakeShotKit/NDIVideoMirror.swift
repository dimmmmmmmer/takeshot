...
///
/// The discipline is `MultiviewEncoder`'s and `PlayoutFeeder`'s: the display
/// queue drops a frame here and returns at once, only the newest frame is kept,
/// and the send runs on this queue — never on capture, never on main. NDI's send
/// is synchronous and compresses the frame inside it, so it is real work; what
/// keeps that off the paths that matter is that it happens here and that a pass
/// which cannot keep up REPLACES the pending frame instead of queueing behind
/// it. The feed falls to fewer frames rather than to older ones, which is the
/// right failure for a monitor.
///
...
/// no NDI consumer at all — an idle app pays nothing, exactly as the multiview
/// encoder does for the camera grid.
final class NDIVideoMirror: @unchecked Sendable {
    ...
    /// never touched from the capture queue.
