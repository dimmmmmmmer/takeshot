    ...
    var body: some View {
        HStack(spacing: Self.slotSpacing) {
            ...
            menu("timeline.selection", help: L("export_timeline_help")) {
                // **One heading over all three, because all three are the
                // same cut in three spellings** (owner: "экспорт тейков –
                // каждый должен экспортить только хорошие; таймлайн со всеми
                // мне зачем? просто на хорошие чтоб разные форматы таймлайна
                // были").
                //
                // It was two sections, because the ALE and the FCPXML carried
                // every take that was shot and the EDL did not — so the one
                // note the owner had asked for would have been false for two
                // items out of three. Making it true was the answer, not
                // splitting the menu: what documents the whole day is the
                // SHIFT REPORT in the menu beside this one, which lists every
                // take good or bad.
