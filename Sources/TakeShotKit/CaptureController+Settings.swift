...
extension CaptureController {
    ...
    private func applyPipelineChange(from oldValue: CaptureSettings) {
        ...
        // A WHOLE group the pipeline has never heard of: the remote is a socket,
        // so flicking its switch or typing in its port field must not rebuild
        // capture mid-take. Masking the group rather than its fields one by one
        // is also what keeps this honest when a field is added to it: the field
        // stays out of the capture config by default, which is the safe
        // direction.
