...
public enum NameField: String, CaseIterable, Sendable {
    ...
    private func accepts(_ scalar: Unicode.Scalar) -> Bool {
        ...
        case .clip, .take, .shot, .scene:
            ...
            // nowhere to put.
            //
            // Scene followed for the same reason (owner: "scene в мете должна
            // принимать только числовые значения"). It stays a STRING rather
            // than becoming an Int, because a scene number can carry leading
            // zeros an operator means — "007" is not 7 on a slate — and the
            // template writes it into the file name verbatim.
