...
final class OffloadTarget {
    ...
    /// Free space on the destination's volume — `VolumeSpace` owns the rule,
    /// and it owns it because this reader and the tile beside it used to
    /// disagree (see there).
    ///
    /// nil means "no number", not "no room", and when neither key gives a
    /// positive one the copy GOES AHEAD. A preflight exists to save an
    /// operator an hour and a half-copy; one that refuses on a number it could
    /// not read costs them the offload instead, and a disk that really is full
    /// fails on the first write with an error that says so.
    private func availableBytes() -> Int64? {
        VolumeSpace.free(of: root)
    }
